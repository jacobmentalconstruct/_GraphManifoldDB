"""
Model Bridge — centralized model interaction boundary via Ollama HTTP.
...
Ownership: src/core/model_bridge/model_bridge.py
    ...
    and the Ollama HTTP API.

Responsibilities:
    - Embedding requests (text → vector) via Ollama /api/embed
...
Design constraints:
    - stdlib HTTP only (urllib.request) — no external dependencies
...
class ModelBridgeConfig:
    """
    Configuration for the Ollama-backed model bridge.

    Controls endpoint location, model selection, timeouts, and
    metadata about the model's capabilities.
    """
...
class ModelBridge(ModelBridgeContract):
    """
    Concrete model bridge — Ollama HTTP backend.
    ...
    def embed(self, request: EmbedRequest) -> EmbedResponse:
        """
        Embed text content into vector representations via Ollama /api/embed.
